...
def printar():
    ...
    Interface
    '''
    texto_botao["text"] = texto
#interface gráfica
janela = Tk()
janela.title("Automatização de pesquisa")
janela.geometry ("350x500")

texto_orientacao = Label(janela, text = "Pesquisas nos sistemas")
texto_orientacao.grid(column=0, row=0, padx= 100 , pady= 20)
botao = Button(janela, text ="Pesquisar no PJe 2.x", command = printar)
botao.grid (column=0, row=1,padx= 10 , pady= 20)
texto_botao = Label(janela, text= "")
texto_botao.grid (column=0, row=2,padx= 10 , pady= 30)
